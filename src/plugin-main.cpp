...
static bool paths_equal(const std::wstring &a, const std::wstring &b) {
    return _wcsicmp(a.c_str(), b.c_str()) == 0;
}

static std::wstring image_path_for_process(HANDLE hProcess) {
    if (!hProcess) return std::wstring();

    DWORD size = MAX_PATH;
    std::wstring buf(static_cast<size_t>(size), L'\0');
    if (QueryFullProcessImageNameW(hProcess, 0, buf.data(), &size)) {
    ...
    buf.assign(static_cast<size_t>(size), L'\0');
    if (!QueryFullProcessImageNameW(hProcess, 0, buf.data(), &size)) return std::wstring();
    buf.resize(static_cast<size_t>(size));
    return normalize_path(buf);
}
...
static std::vector<DWORD> get_pids_for_image_path(const ProcessSnapshot &snap, const std::wstring &normalizedPath) {
    std::vector<DWORD> pids;
    if (normalizedPath.empty()) return pids;

    const std::wstring exeName = file_name_of(normalizedPath);
    const DWORD self = GetCurrentProcessId();
    for (const auto &entry : snap.entries) {
        if (entry.first == 0 || entry.first == self) continue;
        if (_wcsicmp(entry.second.c_str(), exeName.c_str()) != 0) continue;

        HANDLE hProcess = OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, FALSE, entry.first);
        if (!hProcess) continue;

        std::wstring image = image_path_for_process(hProcess);
        CloseHandle(hProcess);
        if (!image.empty() && paths_equal(image, normalizedPath)) pids.push_back(entry.first);
    }
    return pids;
}
...
struct PidWindowSearchContext {
    const std::vector<DWORD> *pids;
    std::vector<HWND> foundWindows;
};

static BOOL CALLBACK EnumWindowsByPidProc(HWND hwnd, LPARAM lParam) {
    PidWindowSearchContext *ctx = reinterpret_cast<PidWindowSearchContext *>(lParam);
    if (!IsWindowVisible(hwnd)) return TRUE;

    DWORD processId = 0;
    GetWindowThreadProcessId(hwnd, &processId);
    if (!processId || !ctx->pids) return TRUE;
...
static std::vector<HWND> find_windows_for_pids(const std::vector<DWORD> &pids) {
    PidWindowSearchContext ctx;
    ctx.pids = &pids;
    if (pids.empty()) return ctx.foundWindows;
...
static void minimize_windows_for_pid(DWORD pid) {
    const std::vector<DWORD> pids{pid};
    for (HWND hwnd : find_windows_for_pids(pids)) {
        ShowWindow(hwnd, SW_MINIMIZE);
    }
}

static void apply_window_state(const std::vector<HWND> &windows, bool minimize, bool allowFocusChange) {
...
static void spawn_target_process_silent(const std::wstring &path, bool minimize, bool allowFocusChange)
{
    ...
    const std::wstring normalized = normalize_path(path);
    const std::wstring matchPath = normalized.empty() ? path : normalized;
    const std::wstring exeName = file_name_of(matchPath);
    ...
    // Match the full image path so two tools that share a file name are not treated as one.
    std::vector<DWORD> runningPids = get_pids_for_image_path(preSnap, matchPath);
    if (!runningPids.empty()) {
        apply_window_state(find_windows_for_pids(runningPids), minimize, allowFocusChange);
        blog(LOG_INFO, "[Kaleido] App already running: %s", exeNameUtf8.c_str());
        return;
    }

    // If we could not open the process to read its path, fall back to a visible window
    // with the same executable name rather than starting a second copy.
    std::vector<HWND> existingWindows = find_windows_for_exe(preSnap, exeName);
    if (!existingWindows.empty()) {
        apply_window_state(existingWindows, minimize, allowFocusChange);
        blog(LOG_INFO, "[Kaleido] App already running: %s", exeNameUtf8.c_str());
        return;
    }
...
static std::vector<std::wstring> collect_normalized_profile_paths() {
    std::vector<std::wstring> paths;
    for (const StartupEntry &entry : collect_startup_entries()) {
        std::wstring normalized = normalize_path(entry.path);
        if (!normalized.empty()) paths.push_back(std::move(normalized));
    }
    return paths;
}
...
static void run_startup_entries(const std::vector<StartupEntry> &entries, bool allowFocusChange)
{
    ...
    for (size_t i = 0; i < entries.size(); ++i) {
        const StartupEntry &entry = entries[i];
...
static void execute_live_startup_sequence(bool allowFocusChange)
{
    ...
    const char *profileName = plugin_settings ? obs_data_get_string(plugin_settings, "active_profile") : nullptr;
    if (!profileName || strlen(profileName) == 0) profileName = "Default Profile";
    blog(LOG_INFO, "[Kaleido] Starting profile '%s' (%zu apps)", profileName, entries.size());

    // The click arrived on the UI thread. Pass that foreground right to the worker
    // so focusing an already-running app is allowed.
    if (allowFocusChange) AllowSetForegroundWindow(ASFW_ANY);
...
static std::vector<HANDLE> open_matching_profile_processes(const std::vector<std::wstring> &paths) {
    std::vector<HANDLE> handles;
    if (paths.empty()) return handles;

    ProcessSnapshot snap = take_process_snapshot();
    std::unordered_set<DWORD> seen;
    const DWORD self = GetCurrentProcessId();
    for (const std::wstring &path : paths) {
        for (DWORD pid : get_pids_for_image_path(snap, path)) {
            if (pid == self || !seen.insert(pid).second) continue;
            HANDLE hProcess = OpenProcess(PROCESS_TERMINATE | SYNCHRONIZE | PROCESS_QUERY_LIMITED_INFORMATION, FALSE, pid);
            if (hProcess) handles.push_back(hProcess);
        }
    }
    return handles;
}

static void request_close_then_kill(std::vector<HANDLE> handles) {
    std::unordered_map<DWORD, HANDLE> byPid;
    const DWORD self = GetCurrentProcessId();
    for (HANDLE hProcess : handles) {
        ...
        if (byPid.find(pid) != byPid.end()) {
            CloseHandle(hProcess);
            continue;
        }
        byPid.emplace(pid, hProcess);
    }

    if (byPid.empty()) return;

    for (const auto &pair : byPid) {
        EnumWindows(PostCloseProc, static_cast<LPARAM>(pair.first));
    }
    ...
    blog(LOG_INFO, "[Kaleido] Closed %zu processes", byPid.size());
}

static void join_launch_worker() {
    if (launch_worker.joinable()) launch_worker.join();
    launch_in_progress = false;
}

// Closes matching profile executables plus anything this session started. Path matching
// is required because an already-running app is not tracked, and a later OBS restart
// would otherwise leave Close Apps / auto-close with nothing to do.
static void execute_live_termination_sequence(bool background)
{
    std::vector<std::wstring> paths = collect_normalized_profile_paths();
    join_launch_worker();
    ...
    std::vector<HANDLE> matched = open_matching_profile_processes(paths);
    handles.insert(handles.end(), matched.begin(), matched.end());
    if (handles.empty()) return;

    if (!background) {
        request_close_then_kill(std::move(handles));
        return;
    }
    ...
    launch_worker = std::thread([handles]() mutable {
        request_close_then_kill(std::move(handles));
        launch_in_progress = false;
    });
}
...
//=============================================================================
    ...
    void connect_ui_signals_to_slots(QPushButton *closeBtn, QPushButton *addAppBtn, QPushButton *removeAppBtn, QPushButton *saveProfileBtn, QPushButton *launchBtn, QPushButton *quitBtn, QPushButton *exportBtn, QPushButton *importBtn, QPushButton *addLoadoutBtn, QPushButton *removeLoadoutBtn) {
        ...
        connect(loadoutCombo, &QComboBox::currentTextChanged, [this](const QString &text) {
            if (text.isEmpty() || text == loadedProfileName) return;
            persist_app_list(loadedProfileName);
            loadedProfileName = text;
            if (plugin_settings) {
...
public slots:
    void AutoSaveConfig() {
        ...
        QString current_prof = loadoutCombo->currentText();
        if (current_prof.isEmpty()) current_prof = QStringLiteral("Default Profile");
        obs_data_set_string(plugin_settings, "active_profile", current_prof.toUtf8().constData());

        QByteArray geom = saveGeometry();
        obs_data_set_string(plugin_settings, "ui_window_geometry", geom.toHex().constData());

        persist_app_list(current_prof);
        loadedProfileName = current_prof;
    }

    void ReloadFromSettings() {
        if (!loadoutCombo || !programList) return;

        loadoutCombo->blockSignals(true);
        programList->clear();
        loadoutCombo->clear();
        loadoutCombo->addItem(QStringLiteral("Default Profile"));
        ...
        if (plugin_settings) {
            ...
            if (profiles_obj) {
                ...
                while (item) {
                    ...
                    if (prof_name && strcmp(prof_name, "Default Profile") != 0) {
                        loadoutCombo->addItem(QString::fromUtf8(prof_name));
                    }
            ...
            if (savedProf && savedProf[0] && loadoutCombo->findText(QString::fromUtf8(savedProf)) >= 0) {
                loadoutCombo->setCurrentText(QString::fromUtf8(savedProf));
            } else {
                loadoutCombo->setCurrentIndex(0);
                obs_data_set_string(plugin_settings, "active_profile", "Default Profile");
            }

            const char *geom_hex = obs_data_get_string(plugin_settings, "ui_window_geometry");
            if (geom_hex && geom_hex[0]) {
                restoreGeometry(QByteArray::fromHex(geom_hex));
            }
        }

        LoadSavedConfig();
        loadedProfileName = loadoutCombo->currentText();
...
private:
    void persist_app_list(const QString &profileName) {
        if (!plugin_settings || profileName.isEmpty() || !programList) return;

        const QByteArray profUtf8 = profileName.toUtf8();
        obs_data_t *profiles_obj = obs_data_get_obj(plugin_settings, "profiles");
        ...
        obs_data_set_array(profiles_obj, profUtf8.constData(), app_array);
        obs_data_array_release(app_array);
    ...
    void OnRemoveProfile() {
        ...
        if (loadoutCombo->currentText() == QStringLiteral("Default Profile")) {
            QMessageBox::information(this, "Delete Profile", "The default profile cannot be deleted.");
            return;
        }
        if (!plugin_settings) return;

        const QString target = loadoutCombo->currentText();
        ...
        obs_data_t *profiles_obj = obs_data_get_obj(plugin_settings, "profiles");
        if (profiles_obj) {
            obs_data_erase(profiles_obj, targetUtf8.constData());
            obs_data_set_obj(plugin_settings, "profiles", profiles_obj);
            obs_data_release(profiles_obj);
        }
    ...
    void OnManualClose() { execute_live_termination_sequence(true); }
    ...
    QCheckBox *enableAutoStartCheck = nullptr;
    QComboBox *loadoutCombo = nullptr;
    QListWidget *programList = nullptr;
    QCheckBox *autocloseCheck = nullptr;
    QString loadedProfileName;
};
//=============================================================================
...
static QPointer<QMenu> kaleido_menu;
static QObject *plugin_qt_ctx = nullptr;
...
static void kaleido_launcher_save(obs_data_t *save_data, bool saving, void *) {
    ...
    if (saving) {
        if (!plugin_settings) return;
        if (launcher_window) launcher_window->AutoSaveConfig();
        obs_data_t *copy = obs_data_create();
        obs_data_apply(copy, plugin_settings);
        obs_data_set_obj(save_data, KALEIDO_SETTINGS_KEY, copy);
    ...
    if (launcher_window) launcher_window->ReloadFromSettings();
}
...
static void on_obs_frontend_event(enum obs_frontend_event event, void *) {
    ...
    if (event == OBS_FRONTEND_EVENT_FINISHED_LOADING) {
        ...
        if (obsMainWin && obsMainWin->menuBar() && !kaleido_menu) {
            ...
            kaleido_menu = customMenu;
            obsMainWin->menuBar()->addMenu(customMenu);
        }

        if (!plugin_qt_ctx) plugin_qt_ctx = new QObject();
        QTimer::singleShot(2500, plugin_qt_ctx, []() {
...
void obs_module_unload(void) {
    ...
    delete plugin_qt_ctx;
    plugin_qt_ctx = nullptr;

    if (launcher_window) {
        launcher_window->AutoSaveConfig();
        launcher_window->close();
    }

    if (kaleido_menu) {
        QMenu *menu = kaleido_menu;
        kaleido_menu = nullptr;
        delete menu;
    }
    ...
    if (plugin_settings) {
        if (obs_data_get_bool(plugin_settings, "auto_close_enabled")) {
            execute_live_termination_sequence(false);
        }
